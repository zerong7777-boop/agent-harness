...
function accountOptions(): string {
  return state.accounts
    .map(
      (account) => `
        <option value="${account.account_id}" ${state.createForm.account_id === account.account_id ? "selected" : ""}>
          ${cleanText(account.title, account.account_id)}
        </option>
      `,
    )
    .join("");
...
function machineOptions(): string {
  return [
    ...
    ...state.machines.map(
      (machine) => `
        <option value="${machine.machine_id}" ${state.createForm.machine_id === machine.machine_id ? "selected" : ""}>
          ${cleanText(machine.title, machine.machine_id)}
        </option>
      `,
    ),
  ].join("");
...
function findLatestReply(detail?: TaskDetail): { text: string; timestamp?: string; status?: string; sessionId?: string } {
  for (const bundle of detail?.sessions ?? []) {
    const text = cleanText(bundle.session?.latest_assistant_text, "");
    if (text) {
      return {
        text,
          ...
          tasks.length
            ? tasks
                .map((task) => {
                  ...
                  return `
                    <button class="task-row ${task.task_id === state.selectedTaskId ? "is-active" : ""}" data-task-id="${task.task_id}" type="button">
                      <div class="task-row-head">
                        <strong>${cleanText(task.title, task.task_id)}</strong>
                      ...
                      <p class="task-row-subtle">${cleanText(task.recommended_next_step || task.memory_summary, "暂无摘要")}</p>
                    </button>
                  `;
                })
                .join("")
...
function renderOverviewTab(task: TaskSummary, detail?: TaskDetail): string {
  ...
  return `
    ...
    <section class="panel-block">
      <div class="section-head">
        <h3>任务摘要</h3>
        <span class="subtle-line">优先级 ${cleanText(task.priority)}</span>
      </div>
      <p>${cleanText(memory?.summary ?? task.memory_summary ?? detail?.task?.summary, "当前还没有摘要。")}</p>
    </section>
    <section class="panel-block">
      <h3>下一步建议</h3>
      <p>${cleanText(memory?.recommended_next_step ?? task.recommended_next_step, "当前没有建议的下一步。")}</p>
    </section>
    <section class="panel-block">
      <h3>最近回复</h3>
      <div class="reply-card">
        <div class="reply-meta">
          <span>主会话 ${cleanText(reply.sessionId, "未连接")}</span>
  ...
  return `
    <section class="panel-block">
      <div class="section-head">
        <div>
          <h3>计划摘要</h3>
          <p class="subtle-line">${cleanText(detail?.plan?.summary, "当前还没有计划摘要。")}</p>
        </div>
                  ...
                  (phase) => `
                    <article class="list-card">
                      <div class="list-card-head">
                        <strong>${cleanText(phase.title)}</strong>
                        <span>${localizeStage(phase.phase_kind)} / ${localizeStatus(phase.status)}</span>
                      </div>
                      <p>${cleanText(phase.summary, "暂无阶段摘要。")}</p>
                    </article>
                  `,
                )
                .join("")
  ...
  return `
    <section class="panel-grid">
      ...
      <article class="info-card">
        ...
        <strong>${cleanText(runtimeIndex?.primary_session_id ?? task.primary_session_id, "未创建")}</strong>
      </article>
      <article class="info-card">
        <span class="label">线程</span>
        <strong>${cleanText(runtimeStatus?.app_server_thread_id, "未附加")}</strong>
      </article>
      <article class="info-card">
    ...
    <section class="panel-block">
      ...
      <div class="reply-card">
        <div class="reply-meta">
          <span>${cleanText(latestReply.sessionId, cleanText(activeSession?.session?.session_id, "未连接"))}</span>
    ...
    <details class="details-panel">
      ...
      <div class="details-grid">
        ...
        <div><span class="label">WS</span><strong>${cleanText(runtimeStatus?.app_server_ws_url ?? runtimeIndex?.app_server_ws_url)}</strong></div>
        <div><span class="label">Port</span><strong>${cleanText(String(runtimeStatus?.app_server_port ?? runtimeIndex?.app_server_port ?? "-"))}</strong></div>
        <div><span class="label">错误</span><strong>${cleanText(runtimeStatus?.last_error_detail ?? runtimeIndex?.last_error_summary, "无")}</strong></div>
      </div>
    </details>
  `;
}
            ...
            ? detail.experiments
                .map(
                  (experiment) => `
                    <article class="list-card">
                      <div class="list-card-head">
                        <strong>${cleanText(experiment.title)}</strong>
                        <span>${localizeStatus(experiment.status)}</span>
                      </div>
                      <p>${cleanText(experiment.summary, "暂无实验摘要。")}</p>
                    </article>
                  `,
                )
                .join("")
          ...
          detail?.findings?.length
            ? detail.findings
                .map(
                  (finding) => `
                    <article class="list-card">
                      <strong>${cleanText(finding.title)}</strong>
                      <p>${cleanText(finding.summary, "暂无结论摘要。")}</p>
                    </article>
                  `,
                )
                .join("")
...
function renderSessionTab(detail?: TaskDetail): string {
  ...
  return `
    <section class="panel-block">
      <h3>会话摘要</h3>
      <p>${cleanText(bundle?.summary?.summary ?? bundle?.session?.summary, "当前还没有会话摘要。")}</p>
    </section>
    <section class="panel-block">
      <h3>最近事件</h3>
      <div class="stack-list">
            ...
            ? events
                .map(
                  (event) => `
                    <article class="list-card">
                      <div class="list-card-head">
                        <strong>${cleanText(event.kind)}</strong>
                        <span>${formatDate(event.timestamp)}</span>
                      </div>
                      <p>${cleanText(event.summary, "暂无摘要。")}</p>
                    </article>
                  `,
                )
                .join("")
...
function renderWorkspace(): string {
  ...
  let panel = "";
  if (state.selectedTab === "overview") panel = renderOverviewTab(task, detail);
  if (state.selectedTab === "plan") panel = renderPlanTab(task, detail);
  if (state.selectedTab === "runtime") panel = renderRuntimeTab(task, detail);
  if (state.selectedTab === "experiment") panel = renderExperimentTab(detail);
  if (state.selectedTab === "session") panel = renderSessionTab(detail);
    ...
    <section class="workspace">
      <header class="workspace-header">
        <div>
          ...
          <h2>${cleanText(task.title, task.task_id)}</h2>
          <p class="summary">${cleanText(task.memory_summary ?? detail?.task?.summary, "暂无摘要。")}</p>
        </div>
...
function bindEvents(): void {
  ...
  app.querySelectorAll<HTMLButtonElement>("[data-tab-id]").forEach((button) => {
    button.addEventListener("click", () => {
      const tab = (button.dataset.tabId as TabId) || "overview";
    ...
    element.addEventListener("click", async () => {
      ...
      if (action === "toggle-archive") {
        ...
        if (task) {
          ...
          await runMutation(
            () => setTerminalState(task.task_id, terminalState),
            terminalState ? "任务已归档。" : "任务已恢复。",
          );
        }
        return;
      }
